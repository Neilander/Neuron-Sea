    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                float clampResult68_g401 = clamp((_BurnInsideNoiseFactor - tex2D(
                                          _BurnNoiseTexture,
                                          ((((tex2D(_BurnNoiseTexture, (temp_output_72_0_g401 * _BurnSwirlNoiseScale)).r - 0.5) * float2(1, 1) * _BurnSwirlFactor) + temp_output_72_0_g401) *
                                              _BurnInsideNoiseScale)).r), 0.0, 1.0);
                float temp_output_15_0_g401 = (((_BurnRadius - distance(temp_output_72_0_g401, _BurnPosition)) + (tex2D(_BurnNoiseTexture, (temp_output_72_0_g401 * _BurnEdgeNoiseScale)).r *
                    _BurnEdgeNoiseFactor)) / max(_BurnWidth, 0.01));
                float clampResult18_g401 = clamp(temp_output_15_0_g401, 0.0, 1.0);
                float3 lerpResult29_g401 = lerp(temp_output_28_0_g401,
                                                (pow(saferPower21_g401, max(_BurnInsideContrast, 0.001)) * (((_BurnInsideNoiseColor).rgb * clampResult68_g401) + (_BurnInsideColor).rgb)),
                                                clampResult18_g401);
