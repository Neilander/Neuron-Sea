...
public class StoryImporter : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        // 选择CSV文件
        EditorGUILayout.LabelField("步骤2: 选择CSV文件", EditorStyles.boldLabel);
        EditorGUILayout.BeginHorizontal();
        EditorGUILayout.TextField("CSV文件路径", csvFilePath);
        if (GUILayout.Button("浏览...", GUILayout.Width(80)))
        {
            string newPath = EditorUtility.OpenFilePanel("选择CSV文件", "", "csv");
        ...
        {
            EditorGUILayout.HelpBox("请选择一个CSV文件", MessageType.Info);

            if (GUILayout.Button("创建CSV模板"))
            {
                ...
                if (!string.IsNullOrEmpty(templatePath))
                {
                    ...
                    {
                        statusMessage = "成功创建CSV模板";
                        csvFilePath = templatePath;
                    }
                    else
                    {
                        statusMessage = "创建CSV模板失败";
                    }
                }
            }
        }
        ...
        EditorGUI.BeginDisabledGroup(targetStoryData == null || string.IsNullOrEmpty(csvFilePath));
        ...
        if (GUILayout.Button("复制到Resources文件夹"))
        {
            CopyToResources();
        }
        EditorGUI.EndDisabledGroup();
    ...
    private void ImportData()
    {
        ...
        // 执行导入
        bool success = targetStoryData.ImportFromCSV(csvFilePath);
    ...
    private void CopyToResources()
    {
        ...
        string destPath = Path.Combine(csvDirPath, fileName);
