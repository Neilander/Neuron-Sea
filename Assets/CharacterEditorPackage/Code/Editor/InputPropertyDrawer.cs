        ...
        if (foldOutProperty.boolValue) {
            ...
            switch (type) {
                case InputElement.InputType.Button:
                {
                    ...
                    if (a_Property.FindPropertyRelative("m_UnityInputType").enumValueIndex != 0) {
                        EditorHelp.PropertyDrawerLineWithVar(a_Property, "m_UnityAxisRecognition", ref pos,
                ...
                case InputElement.InputType.Direction:
                {
                    EditorHelp.PropertyDrawerLineWithVar(a_Property, "m_HorizontalAxisName", ref pos,
                        "In Unity's input manager, what is the name of the axis that this input uses for its horizontal value?");
                    EditorHelp.PropertyDrawerLineWithVar(a_Property, "m_VerticalAxisName", ref pos,
                        "In Unity's input manager, what is the name of the axis that this input uses for its vertical value?");
                    EditorHelp.PropertyDrawerLineWithVar(a_Property, "m_DirectionThreshold", ref pos,
