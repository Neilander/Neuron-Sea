    ...
    Vector3 GetInterpolatedPosition(){
        ...
        switch (m_SpriteInterpolation) {
            ...
            case SpriteInterpolation.Extrapolate:
                position = Vector3.Lerp(m_CurrentFixedUpdatePosition,
                    m_CurrentFixedUpdatePosition + new Vector3(m_CapsuleCollider.GetVelocity().x, m_CapsuleCollider.GetVelocity().y) * Time.fixedDeltaTime, interpolationFactor);
                break;
        }
        return position;
    }
